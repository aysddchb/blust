...
def ant_colony_optimization(flights, num_iterations=7, num_ants=100, evaporation_rate=0.9, alpha=2.0, beta=1.5):
    ...
    pheromones = lil_matrix((num_flights, num_flights))
    total_delay_history = []
    best_flight_order = []
    ...
    for iteration in range(num_iterations):
        for ant in range(num_ants):
            ...
            visited_flights = [current_flight]
            total_delay = 0
            ...
            if total_delay < best_delay_time:
                ...
                for i in range(len(visited_flights) - 1):
                    pheromones[visited_flights[i], visited_flights[i+1]] *= (1 - evaporation_rate)
                    pheromones[visited_flights[i], visited_flights[i+1]] += alpha / (1 + best_delay_time)
